...
export default function Profesoras() {
  ...
  const eliminarLic = async (id: string) => {
    if (!id) { console.error('ID de licencia undefined'); return }
    ...
    if (error) {
      ...
      // Recargar para restaurar si falló
      sb.from('licencias_profesoras').select('*').eq('profesora_id', selId).order('fecha_desde', { ascending: false })
        .then(({ data }) => setLicencias(data || []))
    }
  }
