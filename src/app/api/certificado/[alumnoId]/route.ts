...
export async function GET(
  ...
  try {
    ...
    const { data: curso, error: errCurso } = await sb
      .from('cursos')
      .select('nombre, nivel, dias, hora_inicio, hora_fin, bibliografia, profesoras(nombre, apellido)')
      .eq('id', cursoId)
      .single()
    ...
    // 4. Datos del director del instituto
    ...
    // 5. Calcular carga horaria estimada desde clases registradas
    ...
    // Estimación: cada clase dura según horario del curso
    let hsPorClase = 1.5
    if (curso.hora_inicio && curso.hora_fin) {
      ...
      hsPorClase = Math.round(((h2 * 60 + m2) - (h1 * 60 + m1)) / 60 * 10) / 10
    }
    const cargaHoraria = totalClases > 0
      ? `${Math.round(totalClases * hsPorClase)} hs`
      : '—'

    // 6. Número de certificado (reproducible, basado en IDs)
  ...
  <div class="page">
    <div class="page-inner">
      ...
      <div class="body">
        ...
        ${alumno.dni ? `<div class="cert-dni">DNI ${alumno.dni}</div>` : '<div style="margin-bottom:28px"></div>'}
        ...
        <div class="cert-texto">
          ...
          ${curso.hora_inicio ? `de <strong>${curso.hora_inicio}</strong> a <strong>${curso.hora_fin}</strong>` : ''},
          cumpliendo con los requisitos académicos establecidos por el instituto.
        </div>

        <div class="cert-datos">
          <div class="cert-dato">
            <div class="cert-dato-val">${(curso as any).nivel || 'B1'}</div>
