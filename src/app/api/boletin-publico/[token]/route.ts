// src/app/api/boletin-publico/[token]/route.ts
// Sirve un boletín sin requerir login, usando un token temporal de 48hs.
// El token se genera desde /api/boletin-token y se comparte por WhatsApp.
...
export async function GET(
  ...
  try {
    ...
    if (tokenErr || !tokenData) {
      return new NextResponse(paginaError('Link no válido', 'Este link no existe o fue eliminado.'), {
        headers: { 'Content-Type': 'text/html; charset=utf-8' }, status: 404
      })
    }

    if (new Date(tokenData.expires_at) < new Date()) {
      return new NextResponse(paginaError('Link vencido', 'Este link expiró. Solicitá uno nuevo al instituto.'), {
        headers: { 'Content-Type': 'text/html; charset=utf-8' }, status: 410
      })
    }

    // Extraer datos del token para usar en el boletín
    const alumnoId = tokenData.alumno_id
...
function paginaError(titulo: string, mensaje: string): string {
  ...
  <style>body{font-family:Arial,sans-serif;background:#f0eaf8;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}
  .card{background:#fff;border-radius:20px;padding:40px 32px;text-align:center;max-width:360px;box-shadow:0 8px 40px rgba(101,47,141,.15)}
  h1{color:#652f8d;font-size:22px;margin:0 0 12px}p{color:#5a4d6a;font-size:14px;line-height:1.6;margin:0}
  .icon{font-size:48px;margin-bottom:16px}</style></head>
  <body><div class="card"><div class="icon">${titulo.includes('vencido') ? '⏳' : '🔒'}</div><h1>${titulo}</h1><p>${mensaje}</p></div></body></html>`
}
