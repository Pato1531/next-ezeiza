...
// Mapeo de nota numérica o literal a badge de color
function notaBadge(nota: string | number | null): { label: string; bg: string; color: string } {
  if (nota === null || nota === undefined) return { label: '—', bg: '#f5f5f5', color: '#9b8eaa' }
  const n = String(nota).trim().toUpperCase()
  if (n === 'A' || n === 'A+' || n === 'A-' || Number(nota) >= 9) return { label: n, bg: '#e6f4ec', color: '#2d7a4f' }
  if (n === 'B' || n === 'B+' || n === 'B-' || (Number(nota) >= 7 && Number(nota) < 9)) return { label: n, bg: '#e0f0f7', color: '#1a6b8a' }
  if (n === 'C' || n === 'C+' || n === 'C-' || (Number(nota) >= 5 && Number(nota) < 7)) return { label: n, bg: '#fef3cd', color: '#b45309' }
  if (n === 'D' || n === 'F' || Number(nota) < 5) return { label: n, bg: '#fdeaea', color: '#c0392b' }
  // Catch-all para notas numéricas intermedias
...
export async function GET(
  ...
  try {
    ...
    // 4. Exámenes del curso con la nota del alumno
    ...
    // Filas de exámenes — vacío informativo si no hay
    const filas = examenesConNota.length > 0
      ...
      : `<tr><td colspan="3" style="text-align:center;padding:20px 0;color:#9b8eaa;font-size:13px;">
  ...
  <div class="doc">
    ...
    <div class="head">
      <div class="head-inner">
        <div class="inst-block">
          <div class="inst-logo">${institutoNombre.slice(0,2).toUpperCase()}</div>
          <div>
    ...
    <div class="body">
      ...
      <div class="obs-block">
        ...
        <div class="obs-text">
          ${examenesConNota.map(e => e.observacion).filter(Boolean).join(' ') || 'Sin observaciones registradas para este período.'}
        </div>
      </div>
